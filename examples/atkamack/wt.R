...
names(df_fsh) <- c("year",2:11)
head(df_fsh)
 # Anomalies on the data
maxage=10 # For display only...
pivot_longer(df_fsh,cols = 2:11, names_to = "age", values_to = "wt") %>% group_by(age) %>% 
   ...
   geom_text(size=3) + ylab("Year") + xlab("Age") + ggtitle("Atka mackerel data") +
...
#--------------------------
   # show some examples
  df_res <- fn_get_pred(file='wt',source="Atka mackerel")
  df_res
  pivot_longer(df_res, cols=2:11,names_to = "age", values_to = "wt") %>% group_by(age,source) %>% 
   ...
   ggplot(aes(y=year,x=age,fill=Anomaly,label=round(wt,2))) + geom_tile() + 
     ...
     scale_y_reverse() + theme_minimal(base_size=18) + ggtitle("Atka mackerel model")
