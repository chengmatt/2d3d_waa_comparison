...
map_list <- list(
  ...
  map3 = list("ln_Linf" = factor(NA), "ln_beta" = factor(NA), "rho_c" = factor(NA)) # no cohort par for 2d ar1
)
...
for(model in 1:length(WAA_re_model)) {
  ...
  # save models
  models[[model]] <- waa_model
  
} # end model runs

# save models
save(models, file = here("output", "2d3d_models.RData"))
